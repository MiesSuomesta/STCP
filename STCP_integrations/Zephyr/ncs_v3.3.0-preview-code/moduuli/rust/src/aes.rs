...
impl StcpAesCodec {
    ...
    pub fn encrypt(
        ...
        stcp_dump!("Got raw to encrypt", plaintext);
        ...
        stcp_dump!("Got encrypted", &output);
        output
    } 
    ...
    pub fn decrypt(&self, payload: &[u8]) -> Option<Vec<u8>> {
        ...
        stcp_dump!("Got to decrypt", payload);
        ...
        let ret = self.cipher.decrypt(nonce, ciphertext).ok();

        if let Some(ref data) = ret {
            stcp_dump!("Got to decrypt", data);
        }
