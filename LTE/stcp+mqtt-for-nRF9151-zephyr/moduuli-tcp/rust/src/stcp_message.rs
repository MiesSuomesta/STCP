...
pub fn stcp_message_unpack_frame_from(data: &[u8]) -> (StcpMessageHeader, Vec<u8>) {
    ...
    if data_lenght < (header_size_in_bytes + frame_size as usize) {
      stcp_dbg!("Not a frame: {:?} < {:?} ({:?} // {:?})",
        data_lenght,
        header_size_in_bytes + frame_size as usize,
    ...
    let start = header_size_in_bytes as usize;
    let end   = start + frame_size as usize;

    buffer[0..frame_size as usize].copy_from_slice(&data[start..end]);

    (header, buffer)
}

pub fn stcp_message_frame_from_raw(
    frame_payload_in: &[u8]
) -> (StcpMessageHeader, Vec<u8>, isize) {

    let header_size_in_bytes =
        stcp_message_get_header_size_in_bytes();

    let frame_payload_in_len =
        frame_payload_in.len();

    //
    // Need at least header
    //
    if frame_payload_in_len < header_size_in_bytes {

        stcp_dbg!(
            "RAW FRAME: Not enough data for header"
        );

        return (
            stcp_message_create_header(
                StcpMsgType::Unknown,
                0
            ),
            Vec::new(),
    ...
    //
    let hdr =
        stcp_message_form_a_header_from_data(
            frame_payload_in
        );
    ...
    let expected_total_size =
        header_size_in_bytes + payload_size;

    //
    // Validate frame size
    //
    if frame_payload_in_len < expected_total_size {

        stcp_dbg!(
            "RAW FRAME: Incomplete frame got={} expected={}",
            frame_payload_in_len,
            expected_total_size
        );
