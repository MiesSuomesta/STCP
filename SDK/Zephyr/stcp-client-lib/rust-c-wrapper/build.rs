...
/*    let lib_path = crate_dir
        .join("../../stcp-client-lib/rust-client-lib")
        .join("target")
        .join(&target)
        .join("release");
    println!("cargo:rustc-link-search=native={}", lib_path.display());
    println!("cargo:rustc-link-lib=static=stcpclientlib");
*/
    // 📄 Generoi C-header tiedosto
    let config_path = crate_dir.join("cbindgen.toml");
    let out_path = crate_dir.join("include").join("stcp_client_cwrapper_lib.h");
