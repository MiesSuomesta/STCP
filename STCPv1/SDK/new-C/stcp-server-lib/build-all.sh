...
toggle_target_override() {
    ...
    if [[ "$mode" == "disable" ]]; then
        if grep -q "^\s*target\s*=" "$cfg_file" 2>&1 > /dev/null; then
...
[ "x$cln" == "xclean" ] && (
	...
	doCleanCargo stcp-server-lib/rust-server-lib "RUST server lib"

	doCleanCargo stcp-server-lib/rust-c-wrapper "RUST C wrapper lib"
...
(
	...
	[ "x$checks" != "xnope" ] && 
	        PATH="$PATH" CC="$TOOLCHAIN_CC" LD="$TOOLCHAIN_LD" RANLIB="$TOOLCHAIN_RANLIB" AR="$TOOLCHAIN_AR" \
        	        ...
        	        runCargoInRoot check -p stcpserverlib --target x86_64-unknown-linux-gnu

		echo "🔎 Cargo check stcpserverlib OK.................."

	        PATH="$PATH" CC="$TOOLCHAIN_CC" LD="$TOOLCHAIN_LD" RANLIB="$TOOLCHAIN_RANLIB" AR="$TOOLCHAIN_AR" \
        	        CFLAGS="${CFLAGS:--fPIC}" LDFLAGS="${LDFLAGS:---static}" TARGET="$TARGET" RUSTFLAGS="$RUSTFLAGS" \
        	        runCargoInRoot check -p stcp_server_cwrapper_lib --target x86_64-unknown-linux-gnu

		echo "🔎 Cargo check stcp_server_cwrapper_lib OK.................."
	)
	...
	echo "🧱 Build: RUST server lib.... DONE"
) 2>&1 | tee -a $BLOG
...
(
	source prebuild-config.shinc 
	for tname in ./build/lib/libstcp_server_cwrapper_lib.a ./build/stcp_server
	do
		echo "🔍 Testing wrapper functions from $tname ............."
		bash check_wrapper_exports.sh \
		  $tname \
		  build/include/stcp_server_cwrapper_lib.h
