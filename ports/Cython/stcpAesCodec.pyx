...
class stcpAesCodec:
    ...
    def generate_random_base64(self, length: int):
        merkit = string.ascii_letters + string.digits + string.punctuation + " "
        return ''.join(random.choice(merkit) for _ in range(length)).encode()
    
    def generate_random_base64_as_string(self, length: int) -> str:
        return str(self.generate_random_base64(length))
    ...
    def getPaddedKey(self, paramIn: str) -> str:
        ...
        print(f"Clamped : {tmp}");
        return str(tmp[:STCP_AES_KEY_SIZE_IN_BYTES])
    ...
    def getPaddedIV(self, paramIn: str) -> str:
        ...
        print(f"Clamped : {tmp}");
        return str(tmp[:STCP_AES_IV_SIZE_IN_BYTES])
