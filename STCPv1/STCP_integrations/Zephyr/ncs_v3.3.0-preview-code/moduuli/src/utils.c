...
int stcp_tcp_resolve_and_make_socket(const char *host, const int port, struct zsock_addrinfo **res, int* pfd) {
    
    if (!res || !pfd) {
        return -EBADFD;
    }

    LDBG("STCP: Resolving %s:%d....", host, port);
    int rc = stcp_dns_resolve(host, port, res);
    LDBG("STCP: Resolved? %d && %p", rc, res);
    int fd = -EAGAIN;
    if (rc == 0) {
        if (*res != NULL) {
            struct zsock_addrinfo *p = res;
            fd = STCP_SOCKET_OPEN(p->ai_family, p->ai_socktype, IPPROTO_TCP);
            if (fd < 0) {
...
int stcp_tcp_context_connect_and_shake_hands(struct stcp_ctx *ctx, int timeout_ms)
{
    ...
    struct zsock_addrinfo *res = ctx->fsm.dns_resolved;

    if (!res) {
        stcp_dns_resolve(ctx->ctx_hostname, ctx->ctx_port, &res);
        ctx->fsm.dns_resolved = res;
        LDBG("DNS resolved...");
    } else {
        LDBG("DNS already resolved...");
    }
