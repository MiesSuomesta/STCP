...
static void send_msg(int fd, const void *buf, size_t len) {
    ...
    if(n<0) die("send");
    if((size_t)n!=len){ fprintf(stderr,"short send: %zd/%zu\n",n,len); exit(2); }
}
static ssize_t recv_msg(int fd, void *buf, size_t len) {
    ssize_t n; do n=recv(fd,buf,len,0); while(n<0 && errno==EINTR); return n;
}
static void recv_exact_msg(int fd, void *buf, size_t len) {
    ssize_t n=recv_msg(fd,buf,len); if(n<0) die("recv");
    if((size_t)n!=len){ fprintf(stderr,"short control recv: %zd/%zu\n",n,len); exit(2); }
}
static int stcp_socket(void) { int fd=socket(AF_STCP,SOCK_STREAM,STCP_TCP_PROTO); if(fd<0) die("socket(AF_STCP)"); return fd; }
...
static void server_conn(int fd) {
    for(;;){
        struct ctrl c; ssize_t n=recv_msg(fd,&c,sizeof(c));
        if(n==0) return; if(n<0) die("recv ctrl"); if((size_t)n!=sizeof(c)){fprintf(stderr,"bad ctrl size %zd\n",n);return;}
        ...
        if(c.mode==MODE_TX){
            for(uint32_t i=0;i<count;i++){
                ssize_t r=recv_msg(fd,buf,chunk); if(r<=0){fprintf(stderr,"data recv failed %zd\n",r);free(buf);return;}
                if(calls==0) t0=ns_now(); bytes+=(uint64_t)r; calls++;
            }
...
static void run_server(const char *ip,int port){
    int s=stcp_socket(); struct sockaddr_in a; fill_addr(&a,ip,port); if(bind(s,(struct sockaddr*)&a,sizeof(a))<0)die("bind"); if(listen(s,16)<0)die("listen");
    printf("STCP TCP path bench server %s:%d proto=%d\n",ip,port,STCP_TCP_PROTO); fflush(stdout);
    for(;;){int fd=accept(s,NULL,NULL); if(fd<0){if(errno==EINTR)continue;die("accept");} server_conn(fd); close(fd);} }
static void run_client(const char *ip,int port,size_t chunk,uint32_t count,int rounds){
    int fd=stcp_socket(); struct sockaddr_in a; fill_addr(&a,ip,port); if(connect(fd,(struct sockaddr*)&a,sizeof(a))<0)die("connect");
    char *buf=malloc(chunk); if(!buf)die("malloc"); memset(buf,0x5A,chunk);
    printf("STCP TCP path bench client %s:%d chunk=%zu count=%u total=%.2f MiB rounds=%d\n",ip,port,chunk,count,(double)chunk*count/1048576.0,rounds);
    for(int r=1;r<=rounds;r++){
        ...
        uint64_t rb=0,rc=0,rt0=0,rt1=0; for(uint32_t i=0;i<count;i++){ ssize_t n=recv_msg(fd,buf,chunk); if(n<=0){fprintf(stderr,"RX failed %zd\n",n);exit(3);} if(rc==0)rt0=ns_now(); rb+=(uint64_t)n; rc++; } rt1=ns_now();
...
static void usage(const char *p){fprintf(stderr,"Usage:\n  %s server [bind-ip] [port]\n  %s client <server-ip> [port] [chunk] [count] [rounds]\nDefaults: port=19953 chunk=128044 count=256 rounds=3\n",p,p);exit(2);}
int main(int argc,char **argv){ signal(SIGPIPE,SIG_IGN); if(argc<2)usage(argv[0]); if(!strcmp(argv[1],"server")){run_server(argc>2?argv[2]:"0.0.0.0",argc>3?atoi(argv[3]):19953);return 0;} if(!strcmp(argv[1],"client")){if(argc<3)usage(argv[0]);run_client(argv[2],argc>3?atoi(argv[3]):19953,argc>4?strtoul(argv[4],0,0):128044,argc>5?strtoul(argv[5],0,0):256,argc>6?atoi(argv[6]):3);return 0;} usage(argv[0]); }
