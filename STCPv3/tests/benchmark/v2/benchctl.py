...
def init_run(args: argparse.Namespace) -> None:
    ...
    manifest = {
        ...
        "started_utc": utc_now(),
        "suite": args.suite,
        "matrix_sha256": sha256(matrix_copy),
        "source": {
...
def case_error(case: dict) -> str | None:
    ...
    if int(case["errors"]) != 0:
        return f"errors={case['errors']}"
    if int(case["operations"]) <= 0:
        return f"operations={case['operations']}"
    if case["error_details"]:
        return "error_details is not empty"
    if float(case["elapsed_s"]) <= 0:
        return f"elapsed_s={case['elapsed_s']}"
    return None
...
def ingest(args: argparse.Namespace) -> None:
    ...
    if verdict["status"] != "PASS":
        raise ValueError("only a validated PASS run may be ingested")
    ...
    with db:
        ...
        for path in sorted((run_dir / "cases").glob("*.json")):
            ...
            db.execute("INSERT INTO cases VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
                ...
                c.get("rtt_p99_ms"), c.get("client_cpu_percent"), c.get("max_rss_kib"), json.dumps(c, sort_keys=True)))
...
def approve(args: argparse.Namespace) -> None:
    ...
    verdict = load_json(run_dir / "validation.json")
    if verdict["status"] != "PASS":
        raise ValueError("publication denied: validation is not PASS")
    if not (run_dir / "report" / "index.html").is_file():
        raise ValueError("publication denied: report missing")
    manifest = load_json(run_dir / "run.json")
    manifest.update({"state": "approved", "completed_utc": utc_now()})
    write_json(run_dir / "run.json", manifest)
    files = [path for path in run_dir.rglob("*") if path.is_file() and path.name not in {"publish-approved.json", "SHA256SUMS"}]
    sums = "".join(f"{sha256(path)}  {path.relative_to(run_dir)}\n" for path in sorted(files))
    (run_dir / "SHA256SUMS").write_text(sums)
    approval = {"schema_version": SCHEMA_VERSION, "run_id": run_dir.name,
                "approved_utc": utc_now(), "validation_sha256": sha256(run_dir / "validation.json"),
...
if __name__ == "__main__":
    try:
        ...
        print(f"benchmark-v2: {exc}", file=sys.stderr)
        raise SystemExit(2)
