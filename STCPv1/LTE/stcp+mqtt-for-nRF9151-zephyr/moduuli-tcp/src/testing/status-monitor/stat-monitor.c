...
#define RADIO(val) ((__GAT(val) > 0) ? "CONNECTED" : "IDLE")
        ...
        int ctx_count = (int)atomic_get(&stcp_context_alive_count);

        printk(".--<[ STATUS, %d contexts alive ]>---------------------------------------------------------------------->\n",
            ctx_count);
#if 0
        ...
        printk("| [%llu ms] Modem currnet state : LTE: %s PDN: %s IP: %s RADIO: %s => Connection: %s\n",
            up,
            lte ? "OK" : "NOK",
            pdn ? "OK" : "NOK",
            ip ? "OK" : "NOK",
        ...
        {
            extern atomic_t alloc_cnt;
            extern atomic_t free_cnt;

            static size_t lastFree = 0;
            static size_t lastStack = 0;
            ...
            size_t alloc_bytes = stats.allocated_bytes;
            size_t total_bytes = stats.max_allocated_bytes;

            int diffMem = lastFree - free_bytes;
            int diffStack = lastStack - unused_stack;
                        
            printk("| [%llu ms] Memory usage  : %zu free / %zu alloc / %zu max bytes (Change: %d bytes, %ld Allocs, %ld Frees) Stack: %zu free, (Change: %d bytes)\n",
                up,
                free_bytes,
                alloc_bytes,
                total_bytes,
