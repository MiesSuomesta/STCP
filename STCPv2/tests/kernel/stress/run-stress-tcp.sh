...
BIN="$TEST_ROOT/stress/stcp-stress"
...
"$BIN" server --protocol "$PROTO" --port "$PORT" --payload "$PAYLOAD" & SERVER_PID=$!
sleep 1
echo "=== STCP TCP churn ==="
timeout "${TIMEOUT}s" "$BIN" churn --protocol "$PROTO" --port "$PORT" --clients "$CLIENTS" --iterations "$ITERATIONS" --payload "$PAYLOAD"
echo "=== STCP TCP steady ==="
timeout "${TIMEOUT}s" "$BIN" steady --protocol "$PROTO" --port "$PORT" --clients "$CLIENTS" --duration "$DURATION" --payload "$PAYLOAD"
