...
#if defined(CONFIG_MQTT_LIB_STCP)
	/** Use secure TCP transport (STCP) for MQTT connection. */
	MQTT_TRANSPORT_STCP,
...
#if defined(CONFIG_MQTT_LIB_STCP)
		/** STCP socket transport for MQTT */
		struct {
			/** Socket descriptor */
			int sock;
			/** STCP Context assigned for socket
			 *  See STCP Implementation for details.
			 */
			void *stcp_api_instance;
		} stcp;
#endif /* CONFIG_MQTT_LIB_STCP */
