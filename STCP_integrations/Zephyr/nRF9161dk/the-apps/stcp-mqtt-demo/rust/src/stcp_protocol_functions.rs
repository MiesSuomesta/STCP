...
pub extern "C" fn rust_exported_session_recvmsg(
    ...
    let out_ptr = out_buf_void_ptr as *mut u8;

    let out_slice = match stcp_make_mut_slice(out_ptr, out_maxlen, out_maxlen) {
        Ok(s) => s,
    ...
    if hdr_sz == 0 || hdr_sz > 64 {
        return -EPROTO as isize;
    }

    let mut hdr_buf = [0u8; 64];

    let r1 = helpers::tcp_recv_exact(
        sock,
        &mut hdr_buf[..hdr_sz],
        non_blocking,
    );
    ...
    let hdr = StcpMessageHeader::from_bytes_be(&hdr_buf[..hdr_sz]).unwrap();
    ...
    if sess.in_aes_mode() {
        let decrypted = match sess.aes.as_ref()
          .and_then(|aes| aes.decrypt(&payload))
        {
            Some(d) => d,
            None => return -EPROTO as isize,
        };

        let dec_len = decrypted.len();
        let n = core::cmp::min(dec_len, out_slice.len());
        stcp_dump!("AES MODE: encrypted message", &decrypted);
        out_slice[..n].copy_from_slice(&decrypted[..n]);
        return n as isize;
    } else {

        if pay_len > out_slice.len() {
            return -EMSGSIZE as isize;
        }

        stcp_dump!("PUB MODE: plain message", &payload);
        out_slice[..pay_len].copy_from_slice(&payload);

        return pay_len as isize;
    }
}
