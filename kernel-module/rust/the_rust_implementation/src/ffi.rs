...
use crate::ctx::{get_or_init_ctx, free_ctx_for_socket, with_context_lock};
...
pub extern "C" fn stcp_rust_close(sk: *mut c_void, _timeout: i64) -> c_int {
    unsafe {
        let ctx = get_or_init_ctx(sk);
        if !ctx.is_null() {
            with_context_lock(sk, |c| c.on_close());
...
#[no_mangle]
pub extern "C" fn stcp_rust_shutdown(_ctx: *mut c_void) -> i32 {
    // TODO: oikea logiikka
    0
}
...
pub extern "C" fn stcp_rust_bind(
    ...
    unsafe {
        ...
        with_context_lock(sk, |c| c.on_bind(uaddr, addr_len));
    }
    0
}
...
pub extern "C" fn stcp_rust_listen(sk: *mut c_void, _backlog: c_int) -> c_int {
    unsafe {
        ...
        with_context_lock(sk, |c| c.on_listen());
    }
    0
}
...
pub extern "C" fn stcp_rust_accept(
    ...
    unsafe {
        ...
        with_context_lock(new_sk, |c| c.on_accept_server(_lsk));
    }
    0
}
...
pub extern "C" fn stcp_rust_getname(
    ...
    unsafe {
        ...
        with_context_lock(sk, |c| c.on_getname(uaddr, addr_len, peer));
    }
    0
}
