...
static void rx_thread(void *p1, void *p2, void *p3)
{
    ...
    if (sock == NULL || sock->rx_buffer == NULL || sock->rx_buffer_size == 0) {
        LOG_ERR("RX worker started without heap RX buffer");
        if (sock != NULL) {
    ...
    buffer = sock->rx_buffer;
    buffer_size = sock->rx_buffer_size;

    LOG_DBG("RX worker buffer=%p size=%u stack=%u",
            buffer,
    ...
    while (!atomic_get(&sock->rx_stop) && sock->carrier != NULL && sock->carrier->fd >= 0) {
        ...
        if (sock->socket_type == SOCK_DGRAM) {
            ...
            n = zsock_recvfrom(sock->carrier->fd, buffer, buffer_size, 0,
            ...
            n = zsock_recv(sock->carrier->fd, buffer, buffer_size, 0);
            if (n > 0) {
...
int stcp_v2_rx_start(struct stcp_v2_socket *sock)
{
    ...
    if (sock->rx_buffer == NULL || sock->rx_buffer_size == 0) {
        return -ENOMEM;
    }
