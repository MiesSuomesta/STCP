...
int stcp_testing_connect_peer(struct stcp_api *api, struct stcp_ctx *ctx) {
    ...
    if (the_test_server_addr_resolved ==  NULL) {
        rc = stcp_dns_resolve_testing_target();
        if (rc < 0) {
            LERR("Got error while resolving testing address, rc: %d, errno", 
                    rc, errno);
            return rc;
        }
    }
    ...
    if (api) {
        ...
        if (api->ctx && atomic_get(&(api->ctx->closing))) {
