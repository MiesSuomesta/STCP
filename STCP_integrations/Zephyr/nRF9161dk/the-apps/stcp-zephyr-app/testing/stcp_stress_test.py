...
# =========================
# Socket helpers (NO getaddrinfo)
# =========================

def normalize_ipv4(host: str) -> str:
    # Pakota numerinen IPv4 – EI DNS, EI getaddrinfo
...
async def sock_connect_no_gai(sock: socket.socket, addr, timeout: float) -> None:
    """
    Nonblocking connect WITHOUT getaddrinfo().
    Tämä on kriittinen STCP (proto 253) -tuki.
    """
...
async def sock_recv_exact(loop, sock, n, timeout):
    buf = bytearray()
    while len(buf) < n:
...
async def steady_worker(wid, host, port, proto, msg, timeout, stats):
    loop = asyncio.get_running_loop()
    try:
        stats.connects += 1
    ...
    try:
        while True:
            stats.sends += 1
            await asyncio.wait_for(loop.sock_sendall(sock, msg), timeout)
            await sock_recv_exact(loop, sock, len(msg), timeout)
            stats.recvs += 1
    except asyncio.CancelledError:
        pass
    except Exception as e:
        stats.note_error(f"[steady {wid}] io failed: {e!r}", send=True)
        print(f"[STEADY IO FAIL] {e!r}", flush=True)
    finally:
        sock.close()


async def churn_worker(wid, host, port, proto, msg, timeout, messages_per_conn, stats):
    loop = asyncio.get_running_loop()
    while True:
        try:
            stats.connects += 1
        ...
        try:
            ...
            stats.note_error(f"[churn {wid}] io failed: {e!r}", send=True)
    ...
    for i in range(args.clients):
        if args.mode == "steady":
            tasks.append(asyncio.create_task(
                steady_worker(i, args.host, args.port, args.proto, msg, args.timeout, stats)
            ))
        else:
            tasks.append(asyncio.create_task(
                churn_worker(i, args.host, args.port, args.proto, msg, args.timeout, args.messages_per_conn, stats)
            ))
    ...
    try:
        while time.time() - start < args.duration:
            ...
            print(
                ...
                f"(connect={stats.connect_errors})",
                flush=True,
            )
    finally:
        for t in tasks:
