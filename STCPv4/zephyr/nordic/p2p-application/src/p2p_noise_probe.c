...
int p2p_noise_probe_run(const struct shell *sh, const struct p2p_bench_config *cfg)
{
    ...
    enum {
        P2P_FRAME_SIZE = 512U,
        P2P_NOISE2_SIZE = 4096U,
        P2P_NOISE3_SIZE = 4096U,
    };
    uint8_t *frame = NULL;
    uint8_t *noise2 = NULL;
    uint8_t *noise3 = NULL;
    ...
    frame = k_malloc(P2P_FRAME_SIZE);
    noise2 = k_malloc(P2P_NOISE2_SIZE);
    noise3 = k_malloc(P2P_NOISE3_SIZE);
    if (frame == NULL || noise2 == NULL || noise3 == NULL) {
        rc = -ENOMEM;
        shell_error(sh, "P2P Noise probe buffer allocation failed");
        goto out;
    }
    ...
    if (fd < 0) {
        rc = fd;
        shell_error(sh, "STCP connect failed rc=%d", fd);
        goto out;
    }

    rc = send_ms_frame(fd, ms_id, sizeof(ms_id) - 1U);
    if (rc < 0) goto out;
    rc = recv_ms_frame(fd, frame, P2P_FRAME_SIZE, &n);
    if (rc < 0) goto out;
    ...
    if (rc < 0) goto out;
    rc = stcp_p2p_noise_dialer_message1(noise, frame, P2P_FRAME_SIZE);
    if (rc < 0) goto out;
    n = (size_t)rc;
    ...
    rc = recv_noise_frame(fd, noise2, P2P_NOISE2_SIZE, &n);
    if (rc < 0) goto out;
    shell_print(sh, "  Noise XX m2 : RX %u bytes", (unsigned)n);

    rc = stcp_p2p_noise_dialer_message2(noise, noise2, n, noise3, P2P_NOISE3_SIZE);
    if (rc < 0) goto out;
    n = (size_t)rc;
