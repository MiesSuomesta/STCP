...
class ZephyrSerial:
    ...
    def wait_for_serial_text(self, needle, timeout=30):
        """Wait for asynchronous application output containing needle."""
        if self.ser is None:
            raise AssertionError("serial is not open")
        return self._read_until(lambda s: str(needle) in s, timeout)

    def wait_for_file_text(self, path, needle, timeout=30):
        """Poll a host-side log until it contains needle."""
        end = time.monotonic() + float(timeout)
        last = ""
        while time.monotonic() < end:
            try:
                ...
                last = ""
            if str(needle) in last:
                return last
            time.sleep(0.05)
        raise AssertionError(
            f"timeout after {timeout}s waiting for {needle!r} in {path};\n"
            f"last contents:\n{last[-8000:]}"
        )

    def count_file_text(self, path, needle):
        try:
            with open(path, "r", encoding="utf-8", errors="replace") as f:
                return f.read().count(str(needle))
        except FileNotFoundError:
            return 0
