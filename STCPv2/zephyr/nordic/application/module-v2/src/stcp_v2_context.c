...
struct stcp_v2_socket *stcp_v2_socket_alloc(void)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(sockets); ++i) {
        ...
        sockets[i].rx_buffer_size = CONFIG_STCP_V2_RX_BUFFER_SIZE;
        sockets[i].rx_buffer = k_malloc(sockets[i].rx_buffer_size);
        if (sockets[i].rx_buffer == NULL) {
            memset(&sockets[i], 0, sizeof(sockets[i]));
            sockets[i].fd = -1;
            continue;
        }
...
void stcp_v2_socket_free(struct stcp_v2_socket *sock)
{
    ...
    if (sock->rx_buffer != NULL) {
        k_free(sock->rx_buffer);
        sock->rx_buffer = NULL;
