...
GENERATOR_DIR="${GENERATOR_DIR:-$ROOT/benchmark/stcp-raspberry-generator}"
...
DEPLOY_TARGET="${DEPLOY_TARGET:-www-data@fuji:/var/www/html/public/stcp.fi/benchmarks/raspberry-pi}"
...
run_carrier() {
    ...
    local remote_host="${DEPLOY_TARGET%%:*}"
    local remote_base="${DEPLOY_TARGET#*:}"
    local remote_dir="${remote_base%/}/$carrier"
    local rsync_delete=()
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
        warn "DRY_RUN=1; would generate with: $script"
        if [[ "$PUBLISH" == "1" ]]; then
            warn "DRY_RUN=1; would publish to: $remote_host:$remote_dir/"
        fi
        return 0
    fi
    ...
    # Force the project helper to generate locally only. Publication is
    # performed explicitly below, so it cannot silently be skipped.
    if [[ "$carrier" == "udp" ]]; then
        RESULT_DIR="$RESULT_DIR" \
    ...
    [[ -s "$output_dir/index.html" ]] ||
        die "$carrier generator did not create $output_dir/index.html"

    ok "$carrier generation finished"

    if [[ "$PUBLISH" != "1" ]]; then
        warn "PUBLISH=0; $carrier was generated but not published"
        return 0
    fi

    [[ "$DEPLOY_TARGET" == *:* ]] ||
        die "DEPLOY_TARGET must look like user@host:/absolute/path"

    if [[ "$RSYNC_DELETE" == "1" ]]; then
        rsync_delete=(--delete)
    fi

    log "Publishing $carrier to: $remote_host:$remote_dir/"
    ...
    ok "$carrier published to $remote_host:$remote_dir/"
}

main() {
    cd "$ROOT"

    for cmd in bash find jq tee rsync ssh; do
        need_cmd "$cmd"
    done
