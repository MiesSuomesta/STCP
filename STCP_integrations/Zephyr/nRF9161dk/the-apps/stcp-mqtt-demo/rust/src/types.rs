...
impl PeekRxBuff {
    pub const fn new() -> Self {
        Self { buffer: [0u8; 2048], len: 0 }
    }

    pub fn fill(&mut self, transport: *mut kernel_socket) -> i32 {
        let free = self.buffer.len() - self.len as usize;
        stcp_dbg!("Starting to recv, free space {} bytes..", free);

        if free == 0 {
            return -12; // -ENOMEM
        }

        let recv_len: c_int = 0;
        stcp_dbg!("Starting to recv, {} bytes free", free);

        let at_buff = &mut self.buffer[self.len..];

        let ret = stcp_tcp_recv_once!(
                transport,
                at_buff,
                0 // Non bloking ... => NOT TO BLOCK..
            );
        
        stcp_dbg!("Received {} bytes", recv_len);

        if ret > 0 {
            self.len += ret as usize;
            stcp_dbg!("Received total of {} bytes", self.len);
        }

        ret as i32
    }

    pub fn read_data_to(&mut self, transport: *mut kernel_socket, out: &mut [u8]) -> i32 {
        while self.len < out.len() as usize {
            let rc = self.fill(transport);
            if rc <= 0 {
                return rc;
            }
        }
        ...
        let remaining = self.len - out.len()  as usize;
        if remaining > 0 {
            self.buffer.copy_within(out.len()..self.len as usize, 0);
        }
        self.len = remaining;
        out.len() as i32
    }
...
impl StcpMessageHeader {
    ...
    pub fn from_bytes_be(buf: &[u8]) -> Option<Self> {
        ...
        let msg_type_raw = u32::from_be_bytes(buf[i..i+4].try_into().ok()?);
        let _msg_type = StcpMsgType::from_raw(msg_type_raw);
        i += 4;
