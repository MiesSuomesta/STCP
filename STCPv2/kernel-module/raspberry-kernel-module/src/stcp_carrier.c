	...
	while (!kthread_should_stop()) {
...

		{
			...
			if (unlikely(!rust_ctx)) {
				pr_err_ratelimited("stcp: RX dropped before carrier context wiring carrier=%px owner=%px len=%zd\n",
...
int stcp_carrier_bind(struct stcp_carrier *carrier, u32 address, u16 port)
{
	...
	ret = kernel_bind(carrier->socket, (struct sockaddr *)&socket_address,
...
)
{
	...
	ret = kernel_connect(carrier->socket,
		(struct sockaddr *)&socket_address, sizeof(struct sockaddr_in), flags);
...
int stcp_carrier_start_receiver_thread(struct stcp_carrier *carrier)
{
	...
	if (unlikely(!rust_ctx)) {
		pr_err("stcp-debug-v8: refusing RX start without rust_ctx carrier=%px owner=%px\n",
...
)
{
	...
	if (!carrier->connected) {
		pr_err_ratelimited("stcp-debug-v8: carrier_send rejected disconnected carrier=%px len=%zu\n", carrier, len);
		return -ENOTCONN;
	}
	if (!data && len)
		return -EINVAL;
