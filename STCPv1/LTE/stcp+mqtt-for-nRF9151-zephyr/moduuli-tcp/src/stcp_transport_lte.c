...
extern struct stcp_fsm g_STCP_FSM;
...
int stcp_transport_get_modem_states(int *plte, int *ppdn, int *pip, int *pradio, int *connection_ok)
{
    int lte =   atomic_get(&g_lte_active);
    int pdn =   atomic_get(&g_pdn_active);
    int ip =    atomic_get(&g_ip_active);
    int radio = atomic_get(&g_radio_active);
    int ok = lte && pdn && ip && radio;

#define setItem(from, to)   if (to) { *to = from; } 

    setItem(lte,   plte);
    setItem(pdn,   ppdn);
    setItem(ip,    pip);
    setItem(radio, pradio);
    setItem(ok,    connection_ok);

#undef setItem
    LINFBIG("Modem state: Radio: %s  LTE: %s PDN: %s IP: %s => Connection %s",
...
int stcp_radio_is_active() {
    return atomic_get(&g_radio_active) == 1;
}


int stcp_is_reset_requested() {
    int rr = atomic_get(&reset_requested);
    LINF("Reset requested: %d", rr);
    return rr == 1;
}

void stcp_set_reset_requested() {
    LINF("Setting reset requested ON");
...
void stcp_transport_wakeup_radio(void)
{
    ...
    LINFBIG("Trying to wakeup radio..");

    zsock_sendto(sock, "x", 1, 0,
                 (struct sockaddr*)&addr, sizeof(addr));

    sleep_ms_jitter(2000,150);

    zsock_close(sock);
    LINFBIG("Tried to wake up now...");
}

int stcp_transport_wait_for_radio_connected(int seconds)
{
   // LDBG("Waiting when radio of modem connected (%d sec max)...", seconds);
   if (atomic_get(&g_radio_active)) {
        // LDBG("Radio marked active, no need to wait");
        return 0;
   }

   int rc = k_sem_take(&g_sem_radio_ready, K_SECONDS(seconds));
   if (rc < 0) {
       LERR("Radio wa not connected in %d seconds..", seconds);
       return rc;
   }

   LINFBIG("Radio connected!");
   return 0;
}
...
static void the_lte_event_handler(const struct lte_lc_evt *evt)
{
    ...
    switch (evt->type) {
    case LTE_LC_EVT_PDN:
        LINF("PDN EVT: cid=%d evt=%d esm=%d",
            evt->pdn.cid,
            evt->pdn.type,
            evt->pdn.esm_err);
            ...
            switch (evt->pdn.type) {
                ...
                /* ===================================================== */
                /* PDN ACTIVATED → TÄMÄ = DATA READY                     */
                /* ===================================================== */
                case LTE_LC_EVT_PDN_ACTIVATED:
                    LINFBIG("PDN ACTIVATED ✅ (DATA PATH READY)");

                    atomic_set(&g_pdn_active, 1);
                    atomic_set(&g_ip_active, 1);   // 🔥 TÄSSÄ se tehdään
                ...
                case LTE_LC_EVT_PDN_APN_RATE_CONTROL_ON:
                    LWRN("APN RATE CONTROL ON ⚠️");
                    break;

                case LTE_LC_EVT_PDN_APN_RATE_CONTROL_OFF:
                    LINF("APN RATE CONTROL OFF");
                    break;
                ...
                case LTE_LC_EVT_PDN_ESM_ERROR:
                    LERR("PDN ESM ERROR ❌ code=%d", evt->pdn.esm_err);
                ...
                default:
                    // LDBG("Unhandled PDN evt=%d", evt->pdn.type);
                    break;
            }
    ...
    case LTE_LC_EVT_NW_REG_STATUS:
        ...
        case LTE_LC_NW_REG_UICC_FAIL:
            LERR("LTE: UICC FAIL, starting reconnect...");
    ...
    case LTE_LC_EVT_RRC_UPDATE:
        ...
        if (connected) {
            k_sem_give(&g_sem_radio_ready);
        } else {
            k_sem_reset(&g_sem_radio_ready);
        }
    ...
    case LTE_LC_EVT_MODEM_EVENT:
        LINF("MODEM EVENT: %d", (int)(evt->modem_evt.type));
        break;
...
void dump_modem_full_status(void)
{
    ...
    dump_sim_status_of_command("AT+CGSN=1");
...
static void wait_until_sim_ready(int maxTimes) {
    ...
    for (int i = 0; i < maxTimes; i++) {
        LINF("Waiting SIM ready... %d / %d sec", i, maxTimes);
...
static void stcp_modem_force_apn(void)
{
    ...
    //int ret = nrf_modem_at_cmd(buf, sizeof(buf), 'AT+CGDCONT=1,"IP","%s"', STCP_LTE_APN);
    int ret = nrf_modem_at_cmd(buf, sizeof(buf), STCP_LTE_SET_AT_CMD);
...
#define CHECK_LTE_CODE(err, info, CODE)     \
    err = CODE;                             \
    if (err < 0 ) {                         \
        LERRBIG("[Transport Init] %s, rc: %d, errno: %d", info, err, errno); \
        return err;                         \
    }

#define VOID_LTE_CODE(info, CODE)     \
    CODE; LERRBIG("[Transport Init] %s, rc: %d, errno: %d", info, err, errno)


int stcp_transport_lte_init(void) {
    int err;
    uint8_t cid;
    int esm;
    char apn[32];
    ...
    /* 1. Alusta modem */
    CHECK_LTE_CODE(err,"Initialising modem lib...", nrf_modem_lib_init());

    LDBGBIG("[Transport Init] Modem library init done... ");

    /* 2. Rekisteröi handler ENNEN verkkoon rekisteröitymistä */
    VOID_LTE_CODE("Initialising the_lte_event_handler...", 
            lte_lc_register_handler(the_lte_event_handler));

    LDBGBIG("[Transport Init] Event handler init done... ");

    /* 3. Aktivoi PDN-tapahtumat default-kontekstille */
    CHECK_LTE_CODE(err,"Initialising default PDN...", 
            lte_lc_pdn_default_ctx_events_enable());

    LDBGBIG("[Transport Init] Default PDN enabled.. ");
    
    LDBGBIG("[Transport Init] Wakeing up radio.... ");
    stcp_transport_wakeup_radio();

    CHECK_LTE_CODE(err,"Initialising LTE networking...", 
            lte_lc_connect());

    stcp_transport_wakeup_radio();

    LDBGBIG("[Transport Init] LTE Connecting.. ");

    /* 5. Odota rekisteröitymistä */
    k_sem_take(&g_sem_lte_ready, K_FOREVER);
    LDBGBIG("[Transport Init] LTE Connected!");

    /* 6. Hae default APN */
    CHECK_LTE_CODE(err,"Getting default APN name...", 
            lte_lc_pdn_default_ctx_apn_get(apn, sizeof(apn)));

    LDBGBIG("[Transport Init] Default APN name: %s", apn);
    /* 7. Luo uusi PDP-konteksti */
    CHECK_LTE_CODE(err,"Creating new PDN....", 
            lte_lc_pdn_ctx_create(&cid));

    LDBGBIG("[Transport Init] Created context %d", cid);

    /* 8. Konfiguroi PDP-konteksti */
    CHECK_LTE_CODE(err,"Configuring the PDN....", 
            lte_lc_pdn_ctx_configure(cid, apn, LTE_LC_PDN_FAM_IPV4V6, NULL));

    LDBGBIG("[Transport Init] Configured %s (%d)", apn, cid);
 
    /* 9. Aktivoi PDN-yhteys */
    CHECK_LTE_CODE(err,"Activating the PDN....", 
            lte_lc_pdn_activate(cid, &esm, NULL));
    
    LDBGBIG("[Transport Init] Activated %s (%d)", apn, cid);
    LDBGBIG("[Transport Init] LTE connection ok, APN: %s (CID: %d)", apn, cid);
    return 0;
} 
...
int stcp_transport_init(void)
{
    ...
    LINF("@INIT: resetting semaphores LTE/PDN/IP...");

    if (!onceCalled) {
        onceCalled = 1;

        LINFBIG("[Transport Init] The LTE init....");
        stcp_transport_lte_init();
        LINFBIG("[Transport Init] The LTE init DONE!");

        LINFBIG("[Transport Init] Turnng CSCON on");
        stcp_lte_issue_at_command("AT+CSCON=1");

        LINFBIG("[Transport Init] Checking IMEI....");
        dump_sim_status_of_command("AT+CGSN=1");

        LINFBIG("[Transport Init] Checking monitor.....");
        stcp_lte_issue_at_command("AT%XMONITOR");

        LINFBIG("[Transport Init] Checking CGATT.....");
        stcp_lte_issue_at_command("AT+CGATT?");

        LINFBIG("[Transport Init] Checking CGPADDR.....");
        stcp_lte_issue_at_command("AT+CGPADDR");

        LINFBIG("[Transport Init] Checking CEREG.....");
        stcp_lte_issue_at_command("AT+CEREG?");

        LINFBIG("[Transport Init] Starting PDN phase .....");
        int cnt = 3*60 * 2; // 3 minutes
        int pdn_ok = stcp_transport_pdn_is_active();
        LINFBIG("[Transport Init] PDN ok? %d ...", pdn_ok);
        while (!pdn_ok && cnt > 0) {
            // Wait a bit for state....
            k_msleep(500);
            pdn_ok = stcp_transport_pdn_is_active();
            // LINF("[Transport Init] PDN status: %d ...", pdn_ok);
            cnt--;
        }

        if (pdn_ok) {
            LINFBIG("[Transport Init] PDN is OK.");
        } else {
            LWRNBIG("[Transport Init] PDN is not OK after a while..");
        }
        atomic_set(&g_pdn_active, pdn_ok);
        k_sem_give(&g_sem_pdn_ready);

        cnt = 6*60 * 2; // 6 minutes
        int net_ok = stcp_transport_modem_lte_network_is_up();
        LINFBIG("[Transport Init] Network ok? %d ...", net_ok);
        while (!net_ok && cnt > 0) {
            // Wait a bit for state....
            k_msleep(500);
        ...
        if (net_ok) {
            LINFBIG("[Transport Init] Network is UP..");
            k_sem_give(&network_connected_sem);
        } else {
            LWRNBIG("[Transport Init] Network is not UP after a while..");
            k_sem_give(&network_connected_sem);
        }
        atomic_set(&g_ip_active, net_ok);

        LINFBIG("[Transport Init] Calling platform ready....");
        stcp_platform_mark_ready();

        LINFBIG("[Transport Init] Doing DNS resolving....");
...
*/
    LINFBIG("[Transport Init] Modem Status");
    dump_modem_full_status();
...
int stcp_transport_send(struct stcp_ctx *ctx, const uint8_t *buf, size_t len)
{
    ...
    STCP_REF_COUNT_GET(ctx, "@ send message via rust", return -EACCES; );
        //CONTEXT_LOCK(ctx);
            ret = rust_exported_session_sendmsg(ctx->session, &ctx->ks, buf, len);
        //CONTEXT_UNLOCK(ctx);
    STCP_REF_COUNT_PUT(ctx, "@ out of send message via rust");
...
int stcp_transport_send_iovec(struct stcp_ctx *ctx, const struct msghdr *message)
{
    ...
    STCP_REF_COUNT_GET(ctx, "@ send message iovec via rust", return -EACCES; );
        //CONTEXT_LOCK(ctx);
            ret = rust_exported_session_sendmsg_iovec(ctx->session, &ctx->ks, (void *)message, 0, 1);
        //CONTEXT_UNLOCK(ctx);
    STCP_REF_COUNT_PUT(ctx, "@ out of send message iovec via rust");

    // LDBG("Called send message, rc: %d", ret);
    return ret;
}

int stcp_transport_context_connect(struct stcp_ctx *ctx)
{
    if (!ctx) {
        return -EBADFD;
    }
    ...
    STCP_REF_COUNT_GET(ctx, "@ connect context", return -EACCES );
    CONTEXT_LOCK(ctx);

        struct zsock_addrinfo *dns = NULL;
        int fd = -1;

            ret = stcp_dns_resolve_stcp_context(
                ctx,
                &dns);

            fd = ctx->ks.fd;

        //LDBG("Got DNS info: %p rc=%d fd=%d",
        //    dns, ret, fd);
        ...
        if (dns && fd >= 0) {
            ...
            // LDBG("Connect result: %d", ret);
        } else {
            LERR("Called without DNS data!");
            ret = -EINVAL;
        }

    CONTEXT_UNLOCK(ctx);
    STCP_REF_COUNT_PUT(ctx, "@ end of connect context");
