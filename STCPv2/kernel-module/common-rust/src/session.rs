...
const PARSER_IDLE: usize = 0;
const PARSER_ACTIVE: usize = 1;
const PARSER_PENDING: usize = 2;

/*
 * Exactly one execution context parses a socket at a time.  A concurrent RX
 * arrival does not simply give up: it changes ACTIVE to PENDING.  Before the
 * parser releases ownership it observes PENDING and performs another complete
 * pass.  This closes the classic lost-work race where bytes are appended after
 * the active parser has observed an empty queue but before it clears busy.
 */
fn begin_parser(ctx: &StcpContext) -> bool {
    loop {
        match ctx.parser_state.load(Ordering::Acquire) {
            PARSER_IDLE => {
                if ctx.parser_state.compare_exchange(
                    PARSER_IDLE,
                    PARSER_ACTIVE,
                    Ordering::AcqRel,
                    Ordering::Acquire,
            ...
            PARSER_PENDING => return false,
            _ => {
                ctx.parser_state.store(PARSER_PENDING, Ordering::Release);
                return false;
            }
        }
    }
...
fn finish_parser_pass(ctx: &StcpContext) -> bool {
    loop {
        match ctx.parser_state.load(Ordering::Acquire) {
            PARSER_PENDING => {
                if ctx.parser_state.compare_exchange(
            ...
            PARSER_IDLE => return false,
            _ => {
                ctx.parser_state.store(PARSER_IDLE, Ordering::Release);
                return false;
            }
        }
    }
...
pub fn connect(
    ...
    /*
     * A non-zero carrier means that the platform adapter has already created
     * and connected the real stream transport before calling into Rust.
     *
     * Do not look up the destination in LISTENERS here. LISTENERS is local to
     * one kernel instance and therefore can only represent the in-kernel
     * loopback backend. Requiring a local listener for a remote TCP carrier
     * made every cross-host connection fail with ECONNREFUSED even after
     * kernel_connect() had returned success.
     *
     * For the remote stream path we only need a local pair of queues for the
     * frame parser. Actual bytes are transmitted through inner.carrier and
     * received through stcp_rust_carrier_receive().
     */
    let carrier = ctx.inner.lock().carrier;
    if carrier != 0 {
        let mut inner = ctx.inner.lock();
        ...
        inner.role = crate::crypto::Role::Client;
        inner.peer_handshake_done = false;
        inner.tx_nonce = 0;
        inner.expected_rx_nonce = 0;
        inner.tx_sequence = 0;
        inner.expected_rx_sequence = 0;
    ...
    /*
     * No platform carrier: retain the original paired in-kernel loopback
     * backend. In this mode both endpoints genuinely live in this kernel and
     * the local listener registry is the correct rendezvous mechanism.
     */
    let listener_ptr = {
...
fn process_handshake_frames(ctx: &StcpContext) -> Result<(), StcpError> {
    if !begin_parser(ctx) {
        crate::carrier::debug_event(209, ctx, 1, PARSER_PENDING);
        return Ok(());
    }

    loop {
        let result = process_handshake_frames_once(ctx);
        if result.is_err() {
            ctx.parser_state.store(PARSER_IDLE, Ordering::Release);
            return result;
        }
        if !finish_parser_pass(ctx) {
            return Ok(());
        }
        crate::carrier::debug_event(211, ctx, 1, 0);
    }
}

fn process_handshake_frames_once(ctx: &StcpContext) -> Result<(), StcpError> {
    ...
    loop {
        /* Leave the first post-handshake frame in the stream queue.  TCP may
         * deliver HandshakeDone and DATA in one read; consuming DATA here used
         * to turn a valid first application message into a protocol error. */
        let next_type = {
            let wire = queue.lock();
            if wire.len() < STCP_HEADER_LEN {
                ...
                match peek_header(&wire) {
                    Ok(header) => {
                        ...
                        if wire.len() < frame_len { None } else { Some(header.packet_type) }
                    }
        ...
        let Some(frame)=extract_next_wire_frame(ctx,queue)? else { break; }; match frame.header.packet_type {
...
/// Create a server-side child context for a stream carrier accepted by the
/// platform adapter. The child is not inserted into the listener's logical
/// accept queue: the C adapter owns the accepted TCP socket and completes the
/// carrier/handshake setup synchronously before returning it to userspace.
pub fn create_stream_accepted_child(
    ...
    if listener.proto == 254 {
        return Err(StcpError::Unsupported);
    }

    let local = inner.local.ok_or(StcpError::InvalidState)?;
    let peer = Address { addr: 0, port: 0 };
    let shared = Arc::new(Connection::new());

    drop(inner);

    Ok(Box::new(StcpContext::connected_child(
        listener.proto,
        local,
        peer,
        shared,
...
fn fill_application_buffer(ctx: &StcpContext) -> Result<(), StcpError> {
    ...
    if !begin_parser(ctx) {
        crate::carrier::debug_event(209, ctx, 2, PARSER_PENDING);
        return Ok(());
    }

    loop {
        let result = fill_application_buffer_once(ctx);
        if result.is_err() {
            ctx.parser_state.store(PARSER_IDLE, Ordering::Release);
            return result;
        }
        ...
        crate::carrier::debug_event(211, ctx, 2, 0);
    }
}
