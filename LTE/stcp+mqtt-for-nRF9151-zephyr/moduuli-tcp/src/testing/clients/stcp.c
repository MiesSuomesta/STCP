...
//            sys_rand_get(payload, sizeof(payload));
            generate_strftime_payload(payload, STCP_TORTURE_BUF_SIZE);

            //rc = stcp_send(ctx, payload, sizeof(payload), 0);
            rc = stcp_api_send(api, payload, STCP_TORTURE_BUF_SIZE, 0);
            ...
            rc = stcp_api_recv(api, rx_buff, STCP_TORTURE_BUF_SIZE, 0);
