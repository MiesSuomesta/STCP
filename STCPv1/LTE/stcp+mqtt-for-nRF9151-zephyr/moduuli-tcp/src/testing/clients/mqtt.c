...
int stcp_mqtt_reconnect(struct mqtt_ctx *mqtt_ctx)
{
    ...
    CLIENT_LOCK(&client_lock);
        mqtt_disconnect(mqtt_ctx->client_ptr);

        SLEEP_SEC(1);
        MQTT_API_CLIENT_DEBUG(mqtt_ctx->client_ptr);

        rc = mqtt_connect(mqtt_ctx->client_ptr);
    ...
    /* resubscribe */
    MQTT_API_CLIENT_DEBUG(mqtt_ctx->client_ptr);
    rc = stpc_mqtt_subscribe(mqtt_ctx->client_ptr);
    if (rc) {
...
int mqtt_connect_via_stcp(char *host, int port, struct stcp_mqtt_ctx *mqtt_ctx, struct stcp_api **saveTo)
{
