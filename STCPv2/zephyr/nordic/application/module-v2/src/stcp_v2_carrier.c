...
/*
 * TX intentionally has no large automatic/bounce buffer.
 *
 * The shared Rust core owns the wire frame and zsock_send()/zsock_sendto()
 * consume it synchronously.  Keep this path zero-copy from the Zephyr glue
 * perspective; do not add CONFIG-sized uint8_t arrays to this stack frame.
 */
