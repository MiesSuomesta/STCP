...
static int start_benchmark(const struct shell *sh, const char *name,
                           enum bench_job_mode mode)
{
    int rc;
    ensure_config();

    rc = bench_start_async(&shell_cfg, mode);
    if (rc == -EBUSY) {
        shell_error(sh, "Benchmark already running: %s", bench_async_mode_name());
        return rc;
    }
    if (rc < 0) {
        shell_error(sh, "Unable to start %s benchmark: %d", name, rc);
        return rc;
    }

    shell_print(sh, "%s started in background: %s://%s:%s total=%u chunk=%u inactivity_timeout=%u ms",
                name, bench_transport_name(shell_cfg.transport), shell_cfg.host,
                shell_cfg.port, shell_cfg.total_bytes, shell_cfg.chunk_size,
                shell_cfg.timeout_ms);
    shell_print(sh, "Use 'stcp bench status' or 'stcp bench stop'.");
    return 0;
}

static int cmd_bench_upload(const struct shell *sh, size_t argc, char **argv)
{ ARG_UNUSED(argc); ARG_UNUSED(argv); return start_benchmark(sh, "upload", BENCH_JOB_UPLOAD); }
static int cmd_bench_download(const struct shell *sh, size_t argc, char **argv)
{ ARG_UNUSED(argc); ARG_UNUSED(argv); return start_benchmark(sh, "download", BENCH_JOB_DOWNLOAD); }
static int cmd_bench_full(const struct shell *sh, size_t argc, char **argv)
{ ARG_UNUSED(argc); ARG_UNUSED(argv); return start_benchmark(sh, "full", BENCH_JOB_FULL); }
static int cmd_bench_all(const struct shell *sh, size_t argc, char **argv)
{ ARG_UNUSED(argc); ARG_UNUSED(argv); return start_benchmark(sh, "all", BENCH_JOB_ALL); }

static int cmd_bench_stop(const struct shell *sh, size_t argc, char **argv)
{
    ARG_UNUSED(argc); ARG_UNUSED(argv);
    int rc = bench_stop_async();
    if (rc == -EALREADY) {
        shell_print(sh, "No benchmark is running");
        return 0;
    }
    if (rc < 0) {
        shell_error(sh, "Unable to stop benchmark: %d", rc);
        return rc;
    }
    shell_print(sh, "Stop requested for benchmark '%s'", bench_async_mode_name());
    return 0;
}

static int cmd_bench_status(const struct shell *sh, size_t argc, char **argv)
{
    ARG_UNUSED(argc); ARG_UNUSED(argv);
    if (bench_async_is_running()) {
        shell_print(sh, "Benchmark running : %s", bench_async_mode_name());
        shell_print(sh, "No-progress age   : %u ms", bench_async_last_progress_age_ms());
        shell_print(sh, "Inactivity limit  : %u ms", bench_async_stall_ms());
    } else {
        shell_print(sh, "Benchmark idle");
        shell_print(sh, "Last mode         : %s", bench_async_mode_name());
        shell_print(sh, "Last result       : %d", bench_async_last_result());
    }
    return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(bench_cmds,
    ...
    SHELL_CMD(status, NULL, "Show background benchmark status", cmd_bench_status),
    SHELL_CMD(stop, NULL, "Cancel the active benchmark", cmd_bench_stop),
    SHELL_SUBCMD_SET_END
);
