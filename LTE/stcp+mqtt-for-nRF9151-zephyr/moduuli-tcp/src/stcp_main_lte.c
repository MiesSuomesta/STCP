...
int stcp_lte_reset_everythign(struct stcp_ctx *ctx) {
    ...
    if (stcp_is_context_valid(ctx) > 0) {
        LDBGBIG("Resetting semaphores LTE/PDN/IP...");
...
int stcp_lte_do_full_reset(struct stcp_ctx *ctx, int wait)
{
    ...
    if (ctx) {
        STCP_REF_COUNT_GET(ctx, "@ full reset", LWRN("Context ref not got at full reset!"); );
    ...
    if (ctx) {
        STCP_REF_COUNT_PUT(ctx, "@ end of full reset");
    }
...
int stcp_library_init()
{
    ...
    dump_sim_status();
 	...
 	LDBGBIG("Doign DNS resolving...");
	stcp_dns_resolve(pHostName, pHostPort, NULL);
