...
pub(crate) fn hash(parts:&[&[u8]])->[u8;32]{let mut s=Sha256::new();for p in parts{s.update(p);}s.finish()}
pub(crate) fn hmac_parts(key:&[u8], parts:&[&[u8]])->[u8;32]{let mut k=[0u8;BLOCK];if key.len()>BLOCK{k[..OUT].copy_from_slice(&hash(&[key]));}else{k[..key.len()].copy_from_slice(key);}let mut ipad=[0x36u8;BLOCK];let mut opad=[0x5cu8;BLOCK];for i in 0..BLOCK{ipad[i]^=k[i];opad[i]^=k[i];}let mut inner=Sha256::new();inner.update(&ipad);for p in parts{inner.update(p);}let ih=inner.finish();let mut outer=Sha256::new();outer.update(&opad);outer.update(&ih);outer.finish()}
